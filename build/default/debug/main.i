...
static unsigned char I2C_Read(unsigned char ack) {
    unsigned char d;
    I2C_Wait(); RCEN = 1; while (!BF) { }
    d = SSPBUF;
    I2C_Wait(); ACKDT = ack ? 0 : 1; ACKEN = 1;
    while (ACKEN) { }
    return d;
}
...
static void SRAM_LoadHeader(void) {
    ...
    if (sram_index >= ((236 - 2) / 10)) { sram_index = 0; sram_count = 0; }
}

static void SRAM_SaveHeader(void) {
    SRAM_Write(0, sram_index); _delay((unsigned long)((5)*(20000000UL/4000.0)));
    SRAM_Write(1, sram_count); _delay((unsigned long)((5)*(20000000UL/4000.0)));
}
...
static void SRAM_SaveSample(unsigned char hr, unsigned char min, unsigned char sec,
    ...
    unsigned char base = 2 + (sram_index * 10);
    SRAM_Write(base + 0, hr); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 1, min); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 2, sec); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 3, date); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 4, mon); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 5, yr); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 6, (unsigned char)(temp_x10 & 0xFF)); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 7, (unsigned char)((temp_x10 >> 8) & 0xFF)); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 8, hum); _delay((unsigned long)((2)*(20000000UL/4000.0)));
    SRAM_Write(base + 9, (unsigned char)(light | (alert ? 0x80 : 0))); _delay((unsigned long)((2)*(20000000UL/4000.0)));

    sram_index++;
    if (sram_index >= ((236 - 2) / 10)) sram_index = 0;
    if (sram_count < ((236 - 2) / 10)) sram_count++;
    SRAM_SaveHeader();
}
...
static void LCD_I2C_SendNibble(unsigned char nibble, unsigned char rs) {
    unsigned char val = (nibble & 0xF0) | 0x08 | (rs ? 0x01 : 0);
    I2C_Start();
    I2C_Write(0x4E);
    I2C_Write(val | 0x04); _delay((unsigned long)((25)*(20000000UL/4000000.0)));
    I2C_Write(val & ~0x04); _delay((unsigned long)((50)*(20000000UL/4000000.0)));
    I2C_Stop();
}
...
static void LCD_I2C_SendByte(unsigned char b, unsigned char rs) {
    ...
    unsigned char base = 0x08 | (rs ? 0x01 : 0);
    I2C_Start();
    I2C_Write(0x4E);
    I2C_Write(high | base | 0x04); _delay((unsigned long)((25)*(20000000UL/4000000.0)));
    I2C_Write(high | base); _delay((unsigned long)((50)*(20000000UL/4000000.0)));
    I2C_Write(low | base | 0x04); _delay((unsigned long)((25)*(20000000UL/4000000.0)));
    I2C_Write(low | base); _delay((unsigned long)((50)*(20000000UL/4000000.0)));
    I2C_Stop();
}

static void LCD_Cmd(unsigned char c) { LCD_I2C_SendByte(c, 0); _delay((unsigned long)((2)*(20000000UL/4000.0))); }
static void LCD_Data(unsigned char d) { LCD_I2C_SendByte(d, 1); }

static void LCD_Init(void) {
    _delay((unsigned long)((50)*(20000000UL/4000.0)));
    LCD_I2C_SendNibble(0x30, 0); _delay((unsigned long)((5)*(20000000UL/4000.0)));
    LCD_I2C_SendNibble(0x30, 0); _delay((unsigned long)((150)*(20000000UL/4000000.0)));
    LCD_I2C_SendNibble(0x30, 0); _delay((unsigned long)((150)*(20000000UL/4000000.0)));
    LCD_I2C_SendNibble(0x20, 0); _delay((unsigned long)((150)*(20000000UL/4000000.0)));
    LCD_Cmd(0x28);
    LCD_Cmd(0x0C);
    LCD_Cmd(0x06);
    LCD_Cmd(0x01);
...
static unsigned char DHT22_Read(signed int *temp_x10, unsigned int *hum_x10) {
    ...
    TRISDbits.TRISD0 = 0;
    PORTDbits.RD0 = 0;
    _delay((unsigned long)((18)*(20000000UL/4000.0)));
    PORTDbits.RD0 = 1;
    _delay((unsigned long)((30)*(20000000UL/4000000.0)));
    TRISDbits.TRISD0 = 1;

    timeout = 1000; while (PORTDbits.RD0 && --timeout); if (!timeout) return 1;
    timeout = 1000; while (!PORTDbits.RD0 && --timeout); if (!timeout) return 1;
    timeout = 1000; while (PORTDbits.RD0 && --timeout); if (!timeout) return 1;
    ...
    for (i = 0; i < 5; i++) {
        ...
        for (j = 0; j < 8; j++) {
            timeout = 1000; while (!PORTDbits.RD0 && --timeout); if (!timeout) return 1;
            _delay((unsigned long)((35)*(20000000UL/4000000.0)));
            if (PORTDbits.RD0) {
                data[i] |= (1 << (7 - j));
                timeout = 1000; while (PORTDbits.RD0 && --timeout); if (!timeout) return 1;
            }
...
static void ADC_Init(void) {
    ...
    ADCON1 = 0b10000000; ADCON0 = 0b01000001; _delay((unsigned long)((50)*(20000000UL/4000000.0)));
}

static unsigned int ADC_Read(unsigned char ch) {
    ADCON0 = (unsigned char)((ADCON0 & 0b11000111) | ((ch & 0x07) << 3));
    _delay((unsigned long)((50)*(20000000UL/4000000.0))); ADCON0bits.GO_DONE = 1; while (ADCON0bits.GO_DONE) { }
...
void main(void) {
    ...
    LCD_Goto(1,0); LCD_Print("Station I2C v3  ");
    _delay((unsigned long)((2000)*(20000000UL/4000.0))); LCD_Cmd(0x01); _delay((unsigned long)((2)*(20000000UL/4000.0)));

    UART_Print("# Smart Weather Station v3 (I2C LCD + DHT22 + DS3231)\r\n");
    ...
    while (1) {
        if (take_sample_flag) {
            ...
            PORTBbits.RB6 = (unsigned char)((lm35_x10 > 300) ? 1 : 0);


            LCD_Goto(0, 0);
            sprintf(lcd_buffer, "%02u:%02u:%02u T:%u.%uC", rtc_hr, rtc_min, rtc_sec, lm35_x10 / 10, lm35_x10 % 10);
            LCD_Print(lcd_buffer);

            LCD_Goto(1, 0);
            sprintf(lcd_buffer, "H:%u%% L:%u%% A:%u   ", hum_pct, ldr_pct, PORTBbits.RB6);
            LCD_Print(lcd_buffer);


            UART_Num2(rtc_hr); UART_Write(':'); UART_Num2(rtc_min); UART_Write(':'); UART_Num2(rtc_sec); UART_Write(',');
            UART_Num2(rtc_date); UART_Write('/'); UART_Num2(rtc_mon); UART_Write('/'); UART_Num2(yr); UART_Write(',');
            UART_NumFrac(lm35_x10, 10); UART_Write(',');
            UART_Num(hum_pct); UART_Write(','); UART_Num(ldr_pct); UART_Write(','); UART_Num(PORTBbits.RB6); UART_Print("\r\n");


            SRAM_SaveSample(rtc_hr, rtc_min, rtc_sec,
                            rtc_date, rtc_mon, rtc_yr,
                            (signed int)lm35_x10, hum_pct, (unsigned char)ldr_pct, PORTBbits.RB6);
        }
    }
}
